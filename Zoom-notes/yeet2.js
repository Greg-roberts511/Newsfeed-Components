const employees = [
  { name: "John Doe", role: "Receptionist" },
  { name: "John Perez", role: "Accounting" },
  { name: "Hong Kil Dong", role: "Sales" },
  { name: "Monsieur X", role: "Marketing" },
  { name: "Max Mustermann", role: "Support" },
];

const createEmployeeCard = (employee, parentNode) => {
  const div = document.createElement("div");
  const li = document.createElement("li");
  const h3 = document.createElement("h3");
  const span = document.createElement("span");

  const nameTextNode = document.createTextNode(employee.name);
  const roleTextNode = document.createTextNode(employee.role);

  div.classList.add("employee-card");

  span.appendChild(roleTextNode);
...
const createSWCard = (employee, parentNode) => {
  ...
  const nameTextNode = document.createTextNode(employee.name);
  const roleTextNode = document.createTextNode(employee.height);
